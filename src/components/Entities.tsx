...
export const Entities: FC<{
  entities: Record<string, any>[], isLoading: boolean, isCacheLoading: boolean, entitiesUrl: string
}> =
  ({ entities, isLoading, isCacheLoading, entitiesUrl }) => {
    return isLoading ? <Loader /> :
      <div className="p-relative">
        ...
        <List>
          {
            entities.map(({ url, name, title }) => <ListItem key={url} sx={{ justifyContent: 'center' }}>
              <Link component={RouterLink} to={`/${entitiesUrl}/${getIdFromUrl(url)}`} variant="body2">
                { name || title }
              </Link>
            </ListItem>)
          }
